...
import { Slide, Attachment, Language } from '../types';
...
const OutlineEditor: React.FC<OutlineEditorProps> = ({ topic, initialAttachments, onBack, onGenerateSlides, language }) => {
          ...
          <div className="flex flex-col">
            ...
            {initialAttachments.length > 0 && (
                <span className="text-[10px] text-zinc-500 flex items-center gap-1">
                    {t.attached}: {initialAttachments.length} file(s)
                </span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-3">
            {isGenerating && <span className="text-xs text-purple-400 animate-pulse flex items-center gap-1"><Loader2 size={12} className="animate-spin"/> {t.generating}</span>}
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/3 border-r border-zinc-800 bg-black p-6 overflow-y-auto hidden md:block">
          ...
          <pre className="font-mono text-xs text-green-500/80 whitespace-pre-wrap break-all leading-relaxed">
            {rawOutput || <span className="animate-pulse">{t.waitingForTokens}</span>}
            <div ref={bottomRef} />
          </pre>
        </div>
          ...
          <div className="max-w-3xl mx-auto space-y-6 pb-20">
            ...
            {slides.length === 0 && !parseError && (
                 <div className="flex flex-col items-center justify-center h-64 text-zinc-500 space-y-4">
                    ...
                    <p>{hasStreamStarted ? t.parsing : t.initializing}</p>
                 </div>
            )}
            ...
            {slides.map((slide, index) => (
              <div key={slide.id} className="group bg-zinc-900 border border-zinc-800 rounded-xl p-6 transition-all hover:border-zinc-700 hover:shadow-xl hover:shadow-black/50">
                <div className="flex justify-between items-start mb-4 border-b border-zinc-800 pb-2">
                  <span className="text-xs font-mono text-zinc-500 bg-zinc-950 px-2 py-1 rounded">{t.slideLabel} {index + 1}</span>
                  <button 
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs text-zinc-500 uppercase font-semibold">{t.fieldTitle}</label>
                    <input 
                  ...
                  <div>
                    <label className="text-xs text-zinc-500 uppercase font-semibold">{t.fieldContent}</label>
                    <textarea 
                  ...
                  <div>
                     <label className="text-xs text-zinc-500 uppercase font-semibold flex items-center gap-2">
                        {t.fieldVisual} <span className="text-[10px] bg-purple-900/30 text-purple-300 px-1 rounded">AI</span>
                     </label>
                     <textarea 
