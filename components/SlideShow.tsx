...
const SlideShow: React.FC<SlideShowProps> = ({ slides: initialSlides, onBack, topic, presentationId, language }) => {
  ...
  const t = translations[language].slideShow;
  ...
  const handleConfirmAddSlide = async (description: string) => {
      ...
      try {
          const newSlideData = await generateSingleSlide(topic, description, slides, newSlideIndex, language);
    ...
    <div className="h-full flex flex-col bg-black text-white relative">
      ...
      <div className="h-16 flex items-center justify-between px-6 border-b border-zinc-800 bg-zinc-900/50 relative z-20">
        ...
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 hover:bg-zinc-800 rounded-full transition text-zinc-400 hover:text-white" title={t.backTitle}>
            <ArrowLeft size={20} />
          </button>
        ...
        <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
            ...
            <span className="font-mono text-[10px] text-zinc-500">
                {t.slide} {currentIndex + 1} {t.of} {slides.length}
            </span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className="hidden sm:inline">{t.exportPPTX}</span>
          </button>
        </div>
      </div>
      ...
      <ConfirmationModal 
        ...
        confirmText={t.deleteSlideTitle}
