...
export const generateOutlineStream = async (
  ...
  try {
    ...
    const langInstruction = language === 'zh' 
        ? "IMPORTANT: Provide the 'title' and 'content' fields in Simplified Chinese (简体中文). The 'visualDescription' should be in English for best image generation results, but Chinese is acceptable if that is your preference." 
        : "Provide all content in English.";

    // Add the text prompt
    parts.push({ 
        text: `Create a slide deck outline for: ${topic || 'the provided content'}. ${langInstruction}` 
    });
...
export const generateSingleSlide = async (
    ...
    try {
        ...
        const langInstruction = language === 'zh' 
            ? "IMPORTANT: Write the 'title' and 'content' in Simplified Chinese (简体中文)." 
            : "";
